...
function openOrderModal(furnitureId, furnitureName, furniturePrice) {
    const orderModalBackdrop = document.querySelector('[data-order-modal-backdrop]');
    ...
    try {
        if (furniture) {
            const markup = `
                ...
                <div id="images">
                    <img src="${furniture.images?.[0] || ''}" alt="product-image-1" class="furniture-modal-main-image">
                    <ul class="furniture-modal-sub-images-list">
                        <li class="furniture-modal-sub-images-item">
                            <img src="${furniture.images?.[1] || ''}" alt="product-image-2" class="furniture-modal-sub-images-image">
                        </li>
                        <li class="furniture-modal-sub-images-item">
                            <img src="${furniture.images?.[2] || ''}" alt="product-image-3" class="furniture-modal-sub-images-image">
                        </li>
                    </ul>
                </div>
                <div class="furniture-modal-content">
                    <h2 class="furniture-modal-title" id="name-product">${furniture.name?.replace(/\\"/g, '') || 'Назва недоступна'}</h2>
                    <p class="furniture-modal-text" id="type">${furniture.type || 'Тип недоступний'}</p>
                    <p class="furniture-modal-price" id="price">${furniture.price || 'Ціна недоступна'} грн</p>
                    ...
                    <div style="margin-bottom: 24px;">
                        ...
                        <ul class="furniture-modal-colors-list" id="colors-list">
                            <li class="furniture-modal-colors-item" style="background-color: ${furniture.color?.[0] || '#ccc'};"></li>
                            <li class="furniture-modal-colors-item" style="background-color: ${furniture.color?.[1] || '#ccc'};"></li>
                            <li class="furniture-modal-colors-item" style="background-color: ${furniture.color?.[2] || '#ccc'};"></li>
                        </ul>
                    </div>
                    <p class="furniture-modal-text" id="description">${furniture.description || 'Опис недоступний'}</p>
                    <p class="furniture-modal-text" id="dimensions">Розміри: ${furniture.sizes || 'Розміри недоступні'}</p>
                    <button class="furniture-modal-order-button" type="button" id="order-button" data-order-modal-open data-furniture-id="${furniture._id}" data-furniture-name="${furniture.name}" data-furniture-price="${furniture.price}">Перейти до замовлення</button>
                </div>`;
